...
test('account management enforces scope and first-login security', async () => {
  const state = seedState()
  assert.ok(state.accounts.length >= 5)
  assert.equal(new Set(state.accounts.map(account => account.username)).size, state.accounts.length)
  assert.ok(state.accounts.every(account => account.password && account.status && typeof account.mustChangePassword === 'boolean'))

  const accounts = await readFile(new URL('../src/views/AccountsView.vue', import.meta.url), 'utf8')
  const partner = await readFile(new URL('../src/views/PartnerCompaniesView.vue', import.meta.url), 'utf8')
  const changePassword = await readFile(new URL('../src/views/ChangePasswordView.vue', import.meta.url), 'utf8')
  const store = await readFile(new URL('../src/store.ts', import.meta.url), 'utf8')
  const shell = await readFile(new URL('../src/components/AppShell.vue', import.meta.url), 'utf8')
  const login = await readFile(new URL('../src/views/LoginView.vue', import.meta.url), 'utf8')

  assert.ok(accounts.includes('新增平台账号'))
  assert.ok(store.includes('顶级代理只能为自己代理树下的子代理开号'))
  assert.ok(accounts.includes('确定删除账号'))
  assert.ok(partner.includes('选择合作公司'))
  assert.ok(partner.includes('历史合作公司'))
  assert.ok(changePassword.includes('首次登录需要修改密码'))
  assert.ok(accounts.includes('停用或删除账号只影响登录'))
  assert.ok(store.includes('只有平台管理员可以修改系统设置'))
  assert.ok(shell.includes("label: '账号管理'"))
  assert.ok(!shell.includes('切换演示角色'))
  assert.ok(!login.includes('demoPassword'))
})
