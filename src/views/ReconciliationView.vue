...
<template>
  <div>
    ...
    <section v-if="batch" class="stats-grid">
      <article class="stat-card"><div class="stat-top"><span class="stat-label">本期公司应结</span><span class="stat-icon"><Icon name="scale" :size="16"/></span></div><div class="stat-value" style="font-size:21px">{{ money(payableTotal) }}</div><div class="stat-foot">{{ batch.month }} 核算批次</div></article>
      <article class="stat-card"><div class="stat-top"><span class="stat-label">已登记支付</span><span class="stat-icon"><Icon name="check" :size="16"/></span></div><div class="stat-value" style="font-size:21px">{{ money(paidTotal) }}</div><div class="stat-foot">支付完成率 {{ rate }}%</div></article>
      <article class="stat-card"><div class="stat-top"><span class="stat-label">未结差额</span><span class="stat-icon" style="background:var(--warning-soft);color:var(--warning)"><Icon name="alert" :size="16"/></span></div><div class="stat-value" style="font-size:21px">{{ money(Math.max(0,payableTotal-paidTotal)) }}</div><div class="stat-foot">应结减实结</div></article>
      <article class="stat-card"><div class="stat-top"><span class="stat-label">状态</span><span class="stat-icon"><Icon name="history" :size="16"/></span></div><div class="stat-value" style="font-size:21px">{{ batch.status==='paid'?'已结清':batch.status==='confirmed'?'待支付':'待确认' }}</div><div class="stat-foot">{{ batch.confirmedAt||'尚未确认' }}</div></article>
    </section>
