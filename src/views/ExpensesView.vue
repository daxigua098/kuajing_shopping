...
<template>
  <div>
    ...
    <section class="stats-grid">
      <article class="stat-card"><div class="stat-top"><span class="stat-label">筛选金额合计</span><span class="stat-icon"><Icon name="receipt" :size="16"/></span></div><div class="stat-value" style="font-size:21px">{{ money(totals.all) }}</div><div class="stat-foot">{{ filtered.length }} 笔杂费记录</div></article>
      <article class="stat-card"><div class="stat-top"><span class="stat-label">待公司确认</span><span class="stat-icon" style="background:var(--warning-soft);color:var(--warning)"><Icon name="alert" :size="16"/></span></div><div class="stat-value" style="font-size:21px">{{ money(totals.pending) }}</div><div class="stat-foot">{{ filtered.filter(x=>x.status==='pending').length }} 笔会影响批次确认</div></article>
      <article class="stat-card"><div class="stat-top"><span class="stat-label">已结清报销</span><span class="stat-icon"><Icon name="check" :size="16"/></span></div><div class="stat-value" style="font-size:21px">{{ money(totals.settled) }}</div><div class="stat-foot">公司已全额报销</div></article>
    ...
    <Modal :open="detailOpen" title="杂费报销详情" width="640px" @close="detailOpen=false"><div v-if="selected">
      <div class="row between center"><div><span class="badge info no-dot">{{ selected.expenseMonth }} 结算月</span><h2 style="font-size:19px;margin:9px 0 4px">{{ selected.purpose }}</h2><span class="hint">{{ shopName(selected.shopId) }}</span></div><span class="amount" style="font-size:21px">{{ money(selected.amount,selected.currency) }}</span></div>
