...
const details=computed<SettlementDetail[]>(()=>baseDetails.value.filter(detail=>(statusFilter.value==='all'||detail.status===statusFilter.value)&&(showProtectionPeriod.value||!isProtectedDetail(detail))))
...
function exportBatch(){
  ...
  if(viewMode.value==='rent' && isCompanyUser.value) downloadRows('公司店租明细-'+activeBatch.value.month,['店铺编号','店铺','公司','地区','开店日期','关店日期','存活周期','店铺保护期','计费口径','月租'],companyRentRows.value.map(row=>[row.shop?.code||'',row.shop?.name||'',companyName(row.detail.companyId),row.shop?.region||'',row.shop?.openDate||'',row.shop?.closeDate||'未关店',row.survivalText,row.shop?.protectionPeriodMonths?(row.shop.protectionPeriodName||row.shop.protectionPeriodMonths+'个月保护期'):'0 保护期',row.billing.reason,row.billing.amount]),'csv')
