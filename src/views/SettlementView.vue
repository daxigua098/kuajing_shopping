...
const details=computed<SettlementDetail[]>(()=>baseDetails.value.filter(detail=>statusFilter.value==='all'||detail.status===statusFilter.value))
...
function exportSummary(){
  if(!activeBatch.value)return
  if(viewMode.value==='rent' && isCompanyUser.value) downloadRows('公司店租汇总报表-'+activeBatch.value.month,['公司','一次性店','上月存活店','新店保护店','计费店数','平均存活周期','公司应付店租'],companyRentSummary.value.map(row=>[row.company.name,row.oneTime,row.alive,row.protected,row.count,row.averageSurvival,row.rent]),'csv')
...
<template>
  <div>
    ...
    <template v-if="activeBatch">
      ...
      <div class="callout info" style="margin-top:15px"><Icon name="shield" :size="18"/><div><strong>{{ isCompanyUser ? '公司计费口径' : '核算原则' }}</strong><p v-if="isCompanyUser">公司只按上月存活店铺和新开店铺前 3 个月保护期计算店租；代理内部如何分配给子代理、人头与公司无关，不在公司页面展示。</p><p v-else>上月任一天存活即算整月；关店后次月停止结算；砍头模式仅首月支付；杂费由顶级代理垫付后公司全额报销。</p></div></div>
    </template>
  </div>
</template>
