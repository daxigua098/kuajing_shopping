...
import type { SettlementBatch, SettlementDetail, Shop } from '@/types'
...
import { createSettlementExportFields, type SettlementExportFieldKey } from '@/utils/settlementExport'
...
const settlementExportOpen=ref(false)
const settlementExportFields=reactive(createSettlementExportFields())
...
function settlementShopStatus(shop?:Shop){return shop&&isAliveInMonth(shop,month.value)?'存活':'关店'}
function settlementExportValue(row:{detail:SettlementDetail;shop?:Shop;billing:{amount:number;reason:string}},key:SettlementExportFieldKey):string|number{const shop=row.shop;if(key==='shopCode')return shop?.code||'';if(key==='shopName')return shop?.name||shopName(row.detail.shopId);if(key==='ownerName')return ownerName(row.detail.ownerId);if(key==='openDate')return shop?.openDate||'';if(key==='shopStatus')return settlementShopStatus(shop);if(key==='closeDate')return shop?.closeDate||'未封店';if(key==='company')return companyName(row.detail.companyId);if(key==='region')return shop?.region||'';if(key==='protectionPeriod')return shop?.protectionPeriodMonths?(shop.protectionPeriodName||shop.protectionPeriodMonths+'个月保护期'):'无保护期';if(key==='settlementMode')return modeLabel(row.detail.mode);if(key==='rent')return row.detail.rent;if(key==='expense')return row.detail.expense;if(key==='payable')return row.detail.rent+row.detail.expense;if(key==='agent')return agentName(row.detail.agentId);return row.billing.reason}
function openBatchExport(){if(isCompanyUser.value&&viewMode.value==='rent'){settlementExportOpen.value=true;return}exportBatch()}
function selectAllSettlementExportFields(){settlementExportFields.forEach(field=>field.selected=true)}
function resetSettlementExportFields(){settlementExportFields.forEach(field=>field.selected=field.default)}
function exportCompanySettlementFields(){const fields=settlementExportFields.filter(field=>field.selected);if(!fields.length){window.alert('请至少选择一个导出字段');return};downloadRows('公司店租明细-'+activeBatch.value?.month,fields.map(field=>field.label),companyRentRows.value.map(row=>fields.map(field=>settlementExportValue(row,field.key))),'csv');settlementExportOpen.value=false}
...
<template>
  <div>
    <div class="page-toolbar">
      ...
      <span class="spacer"/>
      <button class="btn secondary" @click="openBatchExport"><Icon name="download" :size="15"/>导出批次明细</button>
    ...
    <template v-if="activeBatch">
      ...
      <div class="page-toolbar"><div class="filter-search"><Icon name="search" :size="15"/><input v-model="search" class="input" placeholder="搜索店铺、人头、代理、用途"/></div><span class="spacer"/><button class="btn secondary" @click="openBatchExport"><Icon name="download" :size="15"/>导出当前明细</button><span class="hint">共 {{ viewMode==='rent' ? (isCompanyUser ? companyRentRows.length : details.filter(d=>d.rent>0).length) : expenseRows.length }} 条明细</span></div>
      <div v-if="viewMode==='rent'" class="table-wrap"><table v-if="isCompanyUser" class="data-table"><thead><tr><th>店铺</th><th>公司</th><th>地区</th><th>开店日期</th><th>关店日期</th><th>店铺状态</th><th>存活周期</th><th>店铺保护期</th><th>计费口径</th><th>公司应付店租</th></tr></thead><tbody><tr v-for="row in companyRentRows" :key="row.detail.id"><td><div class="primary-cell">{{ row.shop?.code }}</div><div class="secondary-line">{{ row.shop?.name }}</div></td><td>{{ companyName(row.detail.companyId) }}</td><td>{{ row.shop?.region }}</td><td>{{ row.shop?.openDate }}</td><td>{{ row.shop?.closeDate||'未关店' }}</td><td><span class="badge" :class="settlementShopStatus(row.shop)==='存活'?'success':'neutral'">{{ settlementShopStatus(row.shop) }}</span></td><td><span class="badge info no-dot">{{ row.survivalText }}</span></td><td>{{ row.shop?.protectionPeriodMonths ? (row.shop?.protectionPeriodName || row.shop?.protectionPeriodMonths+'个月保护期') : '无保护期' }}<div class="secondary-line">{{ row.shop?.protectionPeriodMonths || 0 }} 个月</div></td><td><span class="badge" :class="row.billing.reason==='按月·上月存活'?'success':row.billing.reason==='一次性开店结算'?'warning':'info'">{{ row.billing.reason }}</span></td><td class="amount">{{ money(row.billing.amount) }}</td></tr><tr v-if="!companyRentRows.length"><td colspan="10"><div class="table-empty">没有公司店租明细</div></td></tr></tbody></table><table v-else class="data-table"><thead><tr><th>店铺</th><th>人头 / 代理</th><th>公司</th><th>模式</th><th>店铺状态</th><th>店租</th><th>核算说明</th><th>状态</th></tr></thead><tbody><tr v-for="detail in details.filter(d=>d.rent>0)" :key="detail.id"><td><div class="primary-cell">{{ state.shops.find(s=>s.id===detail.shopId)?.code }}</div><div class="secondary-line">{{ shopName(detail.shopId) }}</div></td><td>{{ ownerName(detail.ownerId) }}<div class="secondary-line">{{ agentName(detail.agentId) }}</div></td><td>{{ companyName(detail.companyId) }}</td><td><span class="mode-chip" :class="{head:detail.mode==='head_fee'}">{{ modeLabel(detail.mode) }}</span></td><td><span class="badge" :class="settlementShopStatus(state.shops.find(s=>s.id===detail.shopId))==='存活'?'success':'neutral'">{{ settlementShopStatus(state.shops.find(s=>s.id===detail.shopId)) }}</span></td><td class="amount">{{ money(detail.rent) }}</td><td style="max-width:220px;color:var(--muted)">{{ detail.reason }}</td><td><span class="badge" :class="detail.status==='paid'?'success':detail.status==='confirmed'?'info':'warning'">{{ detail.status==='paid'?'已支付':detail.status==='confirmed'?'已确认':'待确认' }}</span></td></tr><tr v-if="!details.some(d=>d.rent>0)"><td colspan="8"><div class="table-empty">没有店租明细</div></td></tr></tbody></table></div>
    ...
    <Modal :open="settlementExportOpen" title="选择月度核算导出字段" width="820px" @close="settlementExportOpen=false">
      <template #subtitle><p>默认导出店铺编号、店铺名称、人头名字、开店日期和状态；其他字段勾选后才会导出。</p></template>
      <div class="row between center" style="margin-bottom:12px"><span class="hint">已选择 {{ settlementExportFields.filter(field=>field.selected).length }} 个字段</span><div class="row" style="gap:7px"><button class="btn ghost small" @click="selectAllSettlementExportFields">全选</button><button class="btn secondary small" @click="resetSettlementExportFields">恢复默认</button></div></div>
      <div class="export-field-grid"><label v-for="field in settlementExportFields" :key="field.key" class="export-field" :class="{selected:field.selected}"><input v-model="field.selected" type="checkbox"/><span>{{field.label}}</span><b v-if="field.default">默认</b></label></div>
      <div class="callout info" style="margin-top:14px"><Icon name="shield" :size="17"/><div><strong>状态口径</strong><p>“存活”表示所选结算月份内存在存活天数；“关店”表示该月份没有存活天数。</p></div></div>
      <template #footer><button class="btn secondary" @click="settlementExportOpen=false">取消</button><button class="btn primary" @click="exportCompanySettlementFields"><Icon name="download" :size="15"/>导出已选字段</button></template>
    </Modal>
