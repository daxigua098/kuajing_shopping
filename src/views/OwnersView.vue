...
<template>
  <div>
    ...
    <Modal :open="detailOpen" title="人头完整档案" width="960px" @close="detailOpen=false">
      <div v-if="selected">
        <div class="row between center"><div class="person-cell"><span class="avatar role-sub_agent" style="width:46px;height:46px;font-size:16px">{{ selected.name.slice(0,1) }}</span><span><strong style="font-size:15px">{{ selected.name }}</strong><small>{{ selected.phone || '未填写电话' }} · {{ selected.email || '未填写邮箱' }}</small></span></div><span class="badge" :class="statusMeta[ownerBusinessStatus(selected)].cls" style="height:28px;padding:0 11px">{{ statusMeta[ownerBusinessStatus(selected)].label }}</span></div>
